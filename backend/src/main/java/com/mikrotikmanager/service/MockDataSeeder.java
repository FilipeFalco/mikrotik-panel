...
class MockDataSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        managedPortRepository.save(new ManagedPort(0, "ether1", "Internet", "Link principal", null, null, false, now, now));
        managedPortRepository.save(new ManagedPort(0, "ether2", "Cliente João", "Casa João", "10.10.10.0/24", "dhcp-joao", true, now, now));
        managedPortRepository.save(new ManagedPort(0, "ether3", "Cliente Maria", "Casa Maria", "10.10.20.0/24", "dhcp-maria", true, now, now));
        managedPortRepository.save(new ManagedPort(0, "ether4", "Escritório", "Rede do escritório", "10.10.30.0/24", "dhcp-escritorio", true, now, now));
