...
export function OperationPlanDialog({
  ...
  const confirmDisabledReason = plan ? confirmationDisabledReason(plan, executionEnabled) : null;
  const canConfirm = Boolean(plan && canConfirmOperationPlan(plan, executionEnabled));
    ...
    <div className="modal-backdrop" role="presentation">
      <section className="plan-dialog" role="dialog" aria-modal="true" aria-labelledby="operation-plan-title">
        <div className="dialog-header">
          <div>
            <p className="simulation-notice">{executionEnabled ? 'PREVIEW — nenhuma alteração foi enviada ao MikroTik' : 'SIMULAÇÃO — nenhuma alteração será enviada ao MikroTik'}</p>
        ...
        {loading && !plan ? <div className="plan-loading"><span className="loading-spinner" /><p>Analisando o estado atual somente por leitura…</p></div> : plan && (
          <div className="plan-content">
            <section className="plan-overview" aria-label="Resumo do plano">
              ...
              <div><span>Execução RouterOS</span><strong className={executionEnabled ? 'plan-enabled' : 'plan-disabled'}>{executionEnabled ? 'Disponível após confirmação' : 'Desabilitada pela capability'}</strong></div>
            ...
            {onConfirm && !canConfirm && <div className="notice info plan-phase-notice" role="status"><strong>Confirmação indisponível.</strong><span>{confirmDisabledReason ?? 'O plano não está elegível para execução.'}</span></div>}
            {executionEnabled && canConfirm && <div className="notice info plan-phase-notice"><strong>Confirmação em duas etapas.</strong><span>O botão abaixo abre uma confirmação explícita; somente a confirmação final enviará a intenção segura ao backend, que fará novo snapshot.</span></div>}
          </div>
        )}
        <div className="dialog-actions plan-dialog-actions">
          ...
          >
            {plan.operationType === 'BLOCK_DEVICE' ? 'Confirmar bloqueio' : plan.operationType === 'UNBLOCK_DEVICE' ? 'Confirmar liberação' : plan.operationType === 'SET_PORT_SPEED' ? 'Confirmar limite da porta' : 'Confirmar limite do dispositivo'}
          </button>}
        </div>
      </section>
    </div>
