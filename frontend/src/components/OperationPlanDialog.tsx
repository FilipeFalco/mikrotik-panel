...
import type { OperationPlan, PlanSeverity, PlanState } from '../types';
...
export function OperationPlanDialog({ open, plan, loading, onClose, onRefresh }: OperationPlanDialogProps) {
  useEffect(() => {
    ...
    <div className="modal-backdrop" role="presentation">
      <section className="plan-dialog" role="dialog" aria-modal="true" aria-labelledby="operation-plan-title">
        <div className="dialog-header">
          <div>
            <p className="simulation-notice">SIMULAÇÃO — nenhuma alteração será enviada ao MikroTik</p>
        ...
        {loading && !plan ? <div className="plan-loading"><span className="loading-spinner" /><p>Analisando o estado atual somente por leitura…</p></div> : plan && (
          <div className="plan-content">
            <section className="plan-overview" aria-label="Resumo do plano">
              ...
              <div><span>Alvo</span><strong>{plan.target.displayName ?? plan.target.identifier}</strong><small>{plan.target.macAddress ?? plan.target.interfaceName ?? plan.target.identifier}</small></div>
              <div><span>Ownership</span><strong>{plan.ownership}</strong></div>
              <div><span>Execução RouterOS</span><strong className="plan-disabled">Desabilitada na Fase 3</strong></div>
              <div><span>Revisão futura</span><strong>{plan.readyForFutureExecution ? 'Pré-condições atendidas' : 'Revisão necessária'}</strong></div>
            </section>

            {!plan.changeRequired && <div className="notice info plan-phase-notice"><strong>Nenhuma alteração necessária no snapshot atual.</strong><span>Este resultado é apenas um candidato a no-op; uma fase futura deverá reler e revalidar o estado antes de qualquer execução.</span></div>}
            ...
            <div className="notice info plan-phase-notice"><strong>Execução permanece desabilitada.</strong><span>{plan.executionDisabledReason}</span></div>
          </div>
        )}
