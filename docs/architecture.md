...
Spring Boot (127.0.0.1:8080)
        ...
        └── MikrotikGateway
              ...
              └── RouterOsRestGateway (Fase 2; ainda não implementado)
                       │ HTTPS /rest
                       ▼
                    RouterOS 7
```
...
Na Fase 2, o adaptador somente leitura consultará menus correspondentes à CLI, sem endpoints inventados: `/rest/system/resource`, `/rest/interface`, `/rest/ip/dhcp-server`, `/rest/ip/dhcp-server/lease`, `/rest/queue/simple` e `/rest/ip/firewall/filter`. Alterações futuras serão feitas somente em recursos individuais identificados por `.id` retornado pelo RouterOS.
...
Esta decisão é deliberadamente conservadora: não haverá remoção/edição de leases, filas ou regras sem confirmação explícita de propriedade via comentário `MTMGR:*`. A documentação do DHCP lista `block-access` e o comando `make-static` no menu de leases. [DHCP do RouterOS](https://manual.mikrotik.com/docs/network-management/dhcp/)
...
- `MIKROTIK_VERIFY_SSL=false` será tratado somente no cliente HTTP dedicado ao RouterOS da Fase 2. Nenhuma validação TLS global será desabilitada.
